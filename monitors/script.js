...
async function getDataFromStrapi() {
  ...
  document.getElementById("monitorFetched").innerHTML = output;
}
...
async function postData(token) {
  ...
  const urlmonitor = "http://localhost:1337/api/monitors/";
  ...
  let monitorResponse = await fetch(urlmonitor, {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
  ...
  let monitorJson = await monitorResponse.json();
