...
async function getDataFromStrapi() {
  //Url to Strapi API to fetch all monitor objects
  let url = "http://localhost:1337/api/monitors";
  ...
  document.getElementById("monitorFetch").innerHTML = output;
}
...
async function postData(token) {
  ...
  const monitorUrl = "http://localhost:1337/api/monitors/";
  ...
  let monitorReply = await fetch(monitorUrl, {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
  ...
  let monitorJson = await monitorReply.json();
